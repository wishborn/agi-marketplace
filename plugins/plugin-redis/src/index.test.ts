...
describe("Redis plugin", () => {
  ...
  it("uses GHCR redis images for services", async () => {
    const reg = await testActivate(plugin);
    for (const svc of reg.services) {
      expect(svc.containerImage).toMatch(/^ghcr\.io\/civicognita\/redis:/);
    }
  });

  it("uses GHCR redis images for stack containers", async () => {
    const reg = await testActivate(plugin);
    for (const stack of reg.stacks) {
      expect(stack.containerConfig).toBeDefined();
      expect(stack.containerConfig!.image).toMatch(/^ghcr\.io\/civicognita\/redis:/);
